...
setting.tk_num = GM_getValue('tk_num')
setting.tk_num = setting.tk_num == undefined ? 1 : setting.tk_num
setting.tk_num = isNaN(setting.tk_num) ? 1 : setting.tk_num
...
if (url == '/mycourse/studentstudy') {
  ...
  setting.total &&
    $('<a>', {
      ...
      style: 'margin: 0 25px;',
      html:
        '本课程共' + $('.icon').length + '节，剩余' + $('em:not(.openlock)').length + '节未完成',
    })
      .appendTo('.zt_logo')
      .parent()
      .width('auto')
  ...
  _self.layui.use('layer', function () {
    this.layer.open({
      content:
        '拖动进度条、倍速播放、秒过会导致不良记录！题库在慢慢补充，搜不到的题目系统会在次日进行自动补充',
      title: '超星网课助手提示',
      btn: '我已知悉',
      offset: 't',
      closeBtn: 0,
...
function beforeFind() {
  ...
  setting.div = $(
    '<div style="border: 2px dashed rgb(0, 85, 68); width: 330px; position: fixed; top: 0; right: 0; z-index: 99999; background-color: rgba(70, 196, 38, 0.6); overflow-x: auto;">' +
      ...
      '<div style="font-size: medium;">当前查题接口:' +
      ((setting.tk_num % 2) + 1) +
      '</div>' +
      '<button id = "next_tk">切换为下一个查题接口</button>' +
    ...
    .on('click', 'button, td', function () {
      ...
      if (this.nodeName == 'TD') {
        ...
        setting.tk_num++
        GM_setValue('tk_num', setting.tk_num)
        setting.tk_num = GM_getValue('tk_num')
...
function findAnswer() {
  if (setting.num >= $('.TiMu').length) {
    var arr = setting.lose
      ? ['共有 <font color="red">' + setting.lose + '</font> 道题目待完善（已深色标注）', saveThis]
      : ['答题已完成', submitThis]
    ...
    case 0:
      {
        GM_xmlhttpRequest({
          ...
          data:
            ...
            type +
            '&id=' +
            ($('#workLibraryId').val() || $('#oldWorkId').val()),
          ...
          onload: function (xhr) {
            if (!setting.loop) {
              ...
              if (obj.code) {
                ...
                setting.div
                  .children('div:eq(0)')
                  .html(obj.answer || setting.over + '服务器繁忙，正在重试...')
              }
              setting.div.children('span').html(obj.msg || '')
            } else if (xhr.status == 403) {
              var html = xhr.responseText.indexOf('{')
                ? '请求过于频繁，单击下面的切换题库接口按钮试试吧'
                : $.parseJSON(xhr.responseText).data
              setting.div.children('div:eq(0)').data('html', html).siblings('button:eq(0)').click()
            } else {
              setting.div
                .children('div:eq(0)')
                .html('题库异常，单击下面的切换题库接口按钮试试吧...')
            }
          },
          ontimeout: function () {
            setting.loop &&
              setting.div.children('div:eq(0)').html(setting.over + '服务器超时，正在重试...')
          },
        })
      }
      break
    case 1:
      {
        GM_xmlhttpRequest({
          ...
          data:
            'content=' + encodeURIComponent(question.replace(/^<img src="/, '').replace(/">$/, '')),
          timeout: setting.time,
          onload: function (xhr) {
            if (!setting.loop) {
            } else if (xhr.status == 200) {
    ...
    $input
      .each(function (index) {
        if (this.value == 'true') {
          ...
          Boolean($.inArray(tip, data) + 1 || (type == '1' && str.indexOf(tip) + 1)) ==
...
function goCourse(url) {
  GM_xmlhttpRequest({
    ...
    onload: function (xhr) {
      $.globalEval(
        'location.href = "' + $('.articlename a[href]', xhr.responseText).attr('href') + '";'
      )
    },
  })
}
